# LayoutForge — project guide for Codex

A local-first, single-user macOS desktop app (Tauri) for sketching semantic UI layouts on an
infinite canvas and **exporting a structured Markdown spec** that briefs Codex to generate
...
- Preserves icon identity (`ph:<name>`) so Codex re-imports the exact icon.
